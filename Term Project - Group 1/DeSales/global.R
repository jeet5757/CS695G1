...
large_clusters <- which(clust$csize > 10)
...
central %<>%
  mutate(label = ifelse(group < 1, "", as.character(central$nodes.g2.)))
...
#df$days <- weekdays(as.POSIXlt(df$USER_FOLLOWERS_COUNT))
#dfrm <-table(df[,c("USER_GENDER","days")])
#subdf <- subset(df, city =='new york city' | city =='brooklyn')
dfrm <-table(df[,c("USER_CITY", "USER_GENDER")])
